...
router.put('/goals/:id', requireAuth, async (req, res, next) => {
  try {
    ...
    if (progress_hours !== undefined) {
      updates.push('progress_hours = ?');
      params.push(progress_hours);
    }
...
router.post('/timelogs', requireAuth, async (req, res, next) => {
  try {
    ...
    const newTimelog = await query(
      'SELECT tl.*, g.title as goal_title FROM timelogs tl LEFT JOIN goals g ON tl.goal_id = g.id WHERE tl.id = ?',
      [result.insertId]
    );
...
router.put('/timelogs/:id', requireAuth, async (req, res, next) => {
  try {
    ...
    if (duration_hr !== undefined) {
      updates.push('duration_hr = ?');
      params.push(duration_hr);
    }
    if (category !== undefined) {
      updates.push('category = ?');
      params.push(category);
...
router.get('/stats', requireAuth, async (req, res, next) => {
  try {
    ...
    const goalStatsRows = await query(
      `SELECT
    ...
    const timeStatsRows = await query(
      `SELECT
    ...
    res.json({
      goals: goalStatsRows[0],
      time: timeStatsRows[0],
